...
<html>
    <head>
        <meta charset="utf-8">
        <title>${col.title} - Web Photo Share</title>
    ...
    <body>
        ...
        <section class="container_12" id="main_content_area">
            <c:if test="${not empty requestScope.collectionImages}">
                <c:forEach var="image" items="${requestScope.collectionImages}">
                    <!-- BOF PIN IMAGE -->
                ...
                <c:if test="${empty requestScope.collectionImages}">
                    <div class="error_message">These is no image in this collection</div>
                </c:if>
            <div class="clear"></div>
        </section>
        <div class="pagination" id="page-nav">
            <c:if test="${currentPage != 1}">
                <a href="${baseURL}/ViewCollection?collection=${col.collectionId}&page=${currentPage - 1}">Previous</a>
            </c:if>
            <c:forEach begin="1" end="${noOfPages}" var="i">
                <c:choose>
                    <c:when test="${currentPage eq i}"><span>${i}</span></c:when>
                    <c:otherwise><a href="${baseURL}/ViewCollection?collection=${col.collectionId}&page=${i}">${i}</a></c:otherwise>
                </c:choose>
            </c:forEach>
            <c:if test="${currentPage lt noOfPages}">
                <a class="pager-next" href="${baseURL}/ViewCollection?collection=${col.collectionId}&page=${currentPage + 1}">Next</a>
            </c:if>
        </div>
