        ...
        <section id="mainpage">
            <div class="container_12">
                ...
                <div class="grid_9">
                    <div class="actionBar">
                        <ul class="table-menu">
                            <li class="first-menu"><a class="link_text_black active" href="UserManage" title=""><span>Users</span></a></li>
                        ...
                        <div class="adminSearch">
                            <form method="post" id="searchUser" action="SearchUser">
                        ...
                        <tbody>
                            <c:forEach var="u" items="${users}"> 
                                <tr>
                                    ...
                                    <td>${u.email}</td>
                                    <c:set var="status" value="${u.userStatus}"/>
                                    <c:if test="${status==0}">
                                        <td>Validating</td>
                                    </c:if>
                                    <c:if test="${status==1}">
                    ...
                    <div class="pagination" id="page-nav" style="margin-bottom: 20px;">
                        <c:if test="${currentPage != 1}">
                            <a href="${baseURL}/admin/UserManage?page=${currentPage - 1}">Previous</a>
                        </c:if>
                        <c:forEach begin="1" end="${noOfPages}" var="i">
                            <c:choose>
                                <c:when test="${currentPage eq i}"><span>${i}</span></c:when>
                                <c:otherwise><a href="${baseURL}/admin/UserManage?page=${i}">${i}</a></c:otherwise>
                            </c:choose>
                        </c:forEach>
                        <c:if test="${currentPage lt noOfPages}">
                            <a class="pager-next" href="${baseURL}/admin/UserManage?page=${currentPage + 1}">Next</a>
                        </c:if>
                    </div>
